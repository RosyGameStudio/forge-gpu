/*
 * noise.vert.hlsl — Fullscreen quad via SV_VertexID
 *
 * Draws a fullscreen quad without any vertex buffer by using the built-in
 * vertex ID to generate positions.  This is a standard technique for
 * fullscreen passes like noise visualization, post-processing, and blur.
 *
 ...
 *   2--3
 *
 * UV coordinates are generated so the fragment shader can map each pixel
 * to noise space.  The V axis is flipped because texture row 0 is the top
 * of the image, but NDC Y = -1 is the bottom of the screen.
 *
 * No vertex attributes or uniforms needed — positions are computed
 * entirely from SV_VertexID.
 *
...
struct VSOutput
{
    ...
    float2 uv       : TEXCOORD0;   /* UV for noise coordinate mapping  */
};
...
VSOutput main(uint vertex_id : SV_VertexID)
{
     ...
     * Without this flip, the noise pattern would appear vertically inverted. */
