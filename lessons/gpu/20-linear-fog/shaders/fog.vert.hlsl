/*
 * fog.vert.hlsl — Vertex shader for fog-integrated Blinn-Phong
 *
 * Identical to Lesson 18's material.vert.hlsl: transforms vertices to
 * both clip space (for rasterization) and world space (for lighting
 * and fog distance calculation in the fragment shader).
 *
 * The fragment shader needs world-space positions for two purposes:
 *   1. Blinn-Phong lighting (view direction, specular half-vector)
 *   2. Fog distance — the distance from each fragment to the camera
 *      determines how much fog is applied
 *
...
VSOutput main(VSInput input)
{
     ...
     * and for computing the fog distance (|eye - P|). */
