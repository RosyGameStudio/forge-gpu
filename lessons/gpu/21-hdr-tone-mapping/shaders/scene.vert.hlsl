/*
 * scene.vert.hlsl — Vertex shader for lit scene (HDR + cascaded shadows)
 *
 * Transforms vertices to clip space for rasterization and to world space
 * for lighting.  Additionally computes 3 light-space positions (one per
 * cascade) for shadow map sampling in the fragment shader.
 *
...
VSOutput main(VSInput input)
{
     ...
     * and for shadow map light-space transforms. */
