/*
 * instanced.frag.hlsl — Fragment shader with Blinn-Phong lighting
 *
 * Identical to Lesson 12's lighting.frag.hlsl.  Instanced rendering changes
 * how vertices reach the shader (per-instance model matrix instead of
 * per-draw uniform), but the fragment shader sees the same interpolated
 * data — world-space position, normal, and UVs — so lighting is unchanged.
 *
