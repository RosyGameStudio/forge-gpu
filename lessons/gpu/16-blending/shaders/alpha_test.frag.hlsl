/*
 * alpha_test.frag.hlsl — Fragment shader with alpha cutoff and Blinn-Phong
 *
 * Identical to scene.frag.hlsl except for the clip() call that discards
 * fragments whose alpha falls below alpha_cutoff.  This implements
 * glTF's "MASK" alpha mode — a binary transparency test.
 *
 * clip(x) discards the fragment when x < 0.  So clip(alpha - cutoff)
 * discards when alpha < cutoff.  Surviving fragments are fully opaque
 * and write to the depth buffer normally.
 *
 ...
 *
 * Fragment uniform:
 *   register(b0, space3) -> base color + alpha cutoff + lighting (80 bytes)
 *
...
cbuffer FragUniforms : register(b0, space3)
{
    ...
    float  alpha_cutoff;   /* discard fragment if alpha < this            */
...
float4 main(PSInput input) : SV_Target
{
    ...
    /* ── Alpha test — discard before lighting ───────────────────────── */
    clip(surface_color.a - alpha_cutoff);
