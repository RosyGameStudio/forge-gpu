 ...
 * contribution accumulates on top of the existing downsample data in
 * each mip level.  This progressive accumulation is what creates the
 * characteristic multi-scale glow of Jimenez bloom.
 *
 ...
 *
 * Fragment sampler:
 *   register(t0/s0, space2) -> source mip texture (slot 0)
 *
...
float4 main(PSInput input) : SV_Target
{
    ...
    /* ── 9-tap tent filter ──────────────────────────────────────────── */
    /* The tent (bilinear) kernel is separable: it can be decomposed
     * into horizontal and vertical passes, but we sample all 9 taps
     * in one pass for simplicity.  The weights form a tent shape
     * centered on the current texel. */
