...
def diagram_edge_functions():
    ...
    tri = Polygon([v0, v1, v2], closed=True,
                  facecolor=STYLE["surface"], edgecolor="none", alpha=0.6)
    ax.add_patch(tri)

    # Draw the three edges with different accent colors
    edges = [(v0, v1, STYLE["accent1"], "edge 0"),
             (v1, v2, STYLE["accent2"], "edge 1"),
             (v2, v0, STYLE["accent3"], "edge 2")]

    for a, b, color, label in edges:
        ...
        ax.annotate("", xy=mid + n, xytext=mid,
                     arrowprops={"arrowstyle": "->,head_width=0.15",
                                 "color": color, "lw": 1.5},
                     zorder=4)

    # Vertex labels
    for v, name, offset in [(v0, "v0", (0, 0.5)),
                             (v1, "v1", (-0.5, -0.5)),
                             (v2, "v2", (0.5, -0.5))]:
        ax.plot(v[0], v[1], "o", color=STYLE["text"], markersize=7, zorder=5)
        ax.text(v[0] + offset[0], v[1] + offset[1], name,
    ...
    for a, b, color, label in orient_labels:
        ...
        ax.text(mid[0] + n[0], mid[1] + n[1], label,
    ...
    ax.plot(p_in[0], p_in[1], "s", color=STYLE["accent3"],
            markersize=10, zorder=5)
    ax.text(p_in[0] + 0.5, p_in[1] + 0.3, "inside\nall >= 0",
            color=STYLE["accent3"], fontsize=9, ha="left", va="center",
            path_effects=stroke)

    ax.plot(p_out[0], p_out[1], "X", color=STYLE["accent2"],
            markersize=10, zorder=5)
    ax.text(p_out[0] + 0.4, p_out[1] + 0.3, "outside\nmixed signs",
            color=STYLE["accent2"], fontsize=9, ha="left", va="center",
            path_effects=stroke)

    # Title
    ax.text(5.0, 10.5,
            "Edge Function Inside Test",
            color=STYLE["text"], fontsize=16, fontweight="bold",
            ha="center", va="center", path_effects=stroke)

    # Formula below
    ax.text(5.0, -0.3,
            "orient2d(a, b, p) = (b.x-a.x)(p.y-a.y) - (b.y-a.y)(p.x-a.x)",
            color=STYLE["text_dim"], fontsize=10, fontstyle="italic",
            ha="center", va="center", path_effects=stroke)
...
def diagram_barycentric_coords():
    ...
    ax.imshow(rgb, extent=[-0.5, 10.5, -0.5, 10.5], origin="lower",
    ...
    for v, name, offset, color in labels:
        ax.plot(v[0], v[1], "o", color=color, markersize=9,
                markeredgecolor=STYLE["text"], markeredgewidth=1.5, zorder=5)
        ax.text(v[0] + offset[0], v[1] + offset[1], name,
    ...
    # Center annotation
    center = (v0 + v1 + v2) / 3
    ax.plot(center[0], center[1], "+", color=STYLE["text"],
            markersize=12, markeredgewidth=2, zorder=5)
    ax.text(center[0] + 0.8, center[1] + 0.3,
            "centroid\nb0 = b1 = b2 = 1/3",
            color=STYLE["text"], fontsize=9, ha="left", va="center",
            path_effects=stroke)
...
def diagram_rasterization_pipeline():
    ...
    for i in range(len(stages) - 1):
        ...
        ax.annotate("", xy=(x_end, 1.5), xytext=(x_start, 1.5),
                     arrowprops={"arrowstyle": "->,head_width=0.2",
                                 "color": STYLE["text_dim"], "lw": 2})
...
def diagram_alpha_blending():
    ...
    ax.text(5, 6.5, "Source-Over Compositing",
            color=STYLE["text"], fontsize=15, fontweight="bold",
            ha="center", va="center", path_effects=stroke)
    ...
    for y, text in formulas:
        ax.text(5, y, text,
                color=STYLE["accent1"], fontsize=11, fontfamily="monospace",
                ha="center", va="center", path_effects=stroke)

    cases = [
        (2.8, "src.a = 1.0", "fully opaque  ->  dst is replaced",
         STYLE["accent3"]),
        (1.8, "src.a = 0.5", "50% blend  ->  equal mix",
         STYLE["warn"]),
        (0.8, "src.a = 0.0", "fully transparent  ->  dst unchanged",
         STYLE["text_dim"]),
    ]
    for y, left, right, color in cases:
        ax.text(2, y, left, color=color, fontsize=10,
                fontfamily="monospace", ha="center", va="center",
                path_effects=stroke)
        ax.text(7, y, right, color=STYLE["text_dim"], fontsize=9,
                ha="center", va="center", path_effects=stroke)
    ...
    ax2.text(2.75, 6.5, "Visual Example",
             color=STYLE["text"], fontsize=13, fontweight="bold",
             ha="center", va="center", path_effects=stroke)

    # Destination (white background)
    dst = Rectangle((0.5, 3.5), 2, 2,
                     facecolor="#e0e0e0", edgecolor=STYLE["text_dim"],
                     linewidth=1.5, zorder=1)
    ax2.add_patch(dst)
    ax2.text(1.5, 5.8, "dst (white)", color=STYLE["text_dim"],
             fontsize=9, ha="center", va="center", path_effects=stroke)

    # Source (red, semi-transparent)
    src = Rectangle((1.5, 2.5), 2, 2,
                     facecolor="#ff4444", alpha=0.6,
                     edgecolor=STYLE["accent2"], linewidth=1.5, zorder=2)
    ax2.add_patch(src)
    ax2.text(3.8, 2.8, "src (red, a=0.6)", color=STYLE["accent2"],
             fontsize=9, ha="left", va="center", path_effects=stroke)

    # Result arrow
    ax2.annotate("", xy=(2.75, 1.2), xytext=(2.75, 2.3),
                 arrowprops={"arrowstyle": "->,head_width=0.15",
                             "color": STYLE["warn"], "lw": 2})
    ...
    result = Rectangle((1.75, 0.0), 2, 1,
                        facecolor=result_color, edgecolor=STYLE["warn"],
                        linewidth=2, zorder=2)
    ax2.add_patch(result)
    ax2.text(2.75, 0.5, "result", color=STYLE["bg"],
...
def diagram_bounding_box():
    ...
    # Bounding box
    bb_min = np.array([min(v0[0], v1[0], v2[0]),
                       min(v0[1], v1[1], v2[1])])
    bb_max = np.array([max(v0[0], v1[0], v2[0]),
                       max(v0[1], v1[1], v2[1])])
    ...
    for gx in range(int(bb_min[0]), int(bb_max[0]) + 1):
        for gy in range(int(bb_min[1]), int(bb_max[1]) + 1):
            ...
            if inside:
                ax.plot(px, py, ".", color=STYLE["accent3"],
    ...
    bb_rect = Rectangle(
        ...
        bb_max[0] - bb_min[0] + 1,
        bb_max[1] - bb_min[1] + 1,
        facecolor="none", edgecolor=STYLE["warn"],
        linewidth=2, linestyle="--", zorder=4)
    ax.add_patch(bb_rect)

    # Triangle outline
    tri = Polygon([v0, v1, v2], closed=True,
                  facecolor="none", edgecolor=STYLE["accent1"],
                  linewidth=2.5, zorder=5)
    ax.add_patch(tri)

    # Vertex markers
    for v, name in [(v0, "v0"), (v1, "v1"), (v2, "v2")]:
        ax.plot(v[0], v[1], "o", color=STYLE["accent1"],
                markersize=8, zorder=6)
        ax.text(v[0] + 0.4, v[1] + 0.3, name,
                color=STYLE["accent1"], fontsize=11, fontweight="bold",
                ha="left", va="center", path_effects=stroke)

    # Legend
    ax.text(0.2, 0.8, "AABB (bounding box)",
            color=STYLE["warn"], fontsize=10, ha="left", va="center",
            path_effects=stroke)
    ax.plot(0.0, 0.8, "s", color=STYLE["warn"], markersize=6)

    ax.text(0.2, 0.0, "inside (fragment emitted)",
            color=STYLE["accent3"], fontsize=10, ha="left", va="center",
            path_effects=stroke)
...
def diagram_indexed_quad():
    ...
    # --- Left: Vertex buffer ---
    ax.text(2.5, 6.5, "Vertex Buffer (4 vertices)",
    ...
    for i, label in enumerate(vert_data):
        y = 5.0 - i * 1.2
        box = FancyBboxPatch((0.5, y - 0.4), 4, 0.8,
                              boxstyle="round,pad=0.05",
                              facecolor=STYLE["accent1"] + "25",
                              edgecolor=STYLE["accent1"], linewidth=1.5)
        ax.add_patch(box)
        ax.text(0.7, y, f"[{i}]", color=STYLE["accent1"],
                fontsize=10, fontfamily="monospace",
                ha="left", va="center", path_effects=stroke)
        ax.text(2.5, y, label, color=STYLE["text"],
                fontsize=10, ha="center", va="center", path_effects=stroke)

    # --- Right: Index buffer ---
    ax.text(9.5, 6.5, "Index Buffer (6 indices)",
            color=STYLE["text"], fontsize=12, fontweight="bold",
            ha="center", va="center", path_effects=stroke)

    # Triangle 0
    tri0_indices = [("0", STYLE["accent2"]),
                    ("1", STYLE["accent2"]),
                    ("2", STYLE["accent2"])]
    ax.text(9.5, 5.5, "Triangle 0", color=STYLE["accent2"],
            fontsize=10, fontweight="bold",
            ha="center", va="center", path_effects=stroke)
    for j, (idx, color) in enumerate(tri0_indices):
        x = 8.0 + j * 1.0
        box = FancyBboxPatch((x - 0.3, 4.3), 0.6, 0.6,
                              boxstyle="round,pad=0.05",
                              facecolor=color + "25",
                              edgecolor=color, linewidth=1.5)
        ax.add_patch(box)
        ax.text(x, 4.6, idx, color=color, fontsize=11,
                fontfamily="monospace", fontweight="bold",
                ha="center", va="center", path_effects=stroke)

    # Triangle 1
    tri1_indices = [("0", STYLE["accent3"]),
                    ("2", STYLE["accent3"]),
                    ("3", STYLE["accent3"])]
    ax.text(9.5, 3.5, "Triangle 1", color=STYLE["accent3"],
            fontsize=10, fontweight="bold",
            ha="center", va="center", path_effects=stroke)
    for j, (idx, color) in enumerate(tri1_indices):
        x = 8.0 + j * 1.0
        box = FancyBboxPatch((x - 0.3, 2.3), 0.6, 0.6,
                              boxstyle="round,pad=0.05",
                              facecolor=color + "25",
                              edgecolor=color, linewidth=1.5)
        ax.add_patch(box)
        ax.text(x, 2.6, idx, color=color, fontsize=11,
                fontfamily="monospace", fontweight="bold",
                ha="center", va="center", path_effects=stroke)

    # --- Bottom: visual quad ---
    ax.text(5.5, 0.7, "Result: 1 Quad",
            color=STYLE["text"], fontsize=11, fontweight="bold",
            ha="center", va="center", path_effects=stroke)

    qverts = np.array([[3.5, 0.0], [7.5, 0.0], [7.5, -1.0], [3.5, -1.0]])
    tri_a = Polygon([qverts[0], qverts[1], qverts[2]], closed=True,
                     facecolor=STYLE["accent2"] + "30",
                     edgecolor=STYLE["accent2"], linewidth=1.5)
    tri_b = Polygon([qverts[0], qverts[2], qverts[3]], closed=True,
    ...
    # Diagonal line
    ax.plot([qverts[0][0], qverts[2][0]],
            [qverts[0][1], qverts[2][1]],
            color=STYLE["text_dim"], linewidth=1, linestyle="--")
