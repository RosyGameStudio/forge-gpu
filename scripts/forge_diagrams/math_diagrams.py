...
def diagram_white_noise_comparison():
    ...
    for ax, title, noise in zip(axes, titles, noises, strict=False):
        ax.set_facecolor(STYLE["bg"])
...
def diagram_distribution_histogram():
    ...
    for ax, (title, data, color) in zip(axes, datasets, strict=False):
        ax.set_facecolor(STYLE["bg"])
...
def diagram_gradient_noise_concept():
    ...
    for (cx, cy), color, label in zip(
        corners, corner_colors, corner_labels, strict=False
    ):
        gx_abs = cell_x + cx
        gy_abs = cell_y + cy
    ...
    for (cx, cy), color in zip(corners, corner_colors, strict=False):
        gx_abs = cell_x + cx
        gy_abs = cell_y + cy
        ax.plot(
            [gx_abs, sample_x],
...
def diagram_noise_comparison():
    ...
    for ax, title, noise, cmap in zip(axes, titles, noises, cmaps, strict=False):
        ax.set_facecolor(STYLE["bg"])
        ax.imshow(
            noise,
            cmap=cmap,
...
def diagram_fbm_octaves():
    ...
    for ax, octaves in zip(axes, octave_counts, strict=False):
...
def diagram_domain_warping():
    ...
    for ax, strength, label in zip(axes, strengths, labels, strict=False):
        if strength == 0.0:
...
def diagram_sampling_comparison():
    ...
    for ax, (title, xs, ys, color) in zip(axes, datasets, strict=False):
        ax.set_facecolor(STYLE["bg"])
...
def diagram_dithering_comparison():
    ...
    for ax, (title, data) in zip(axes, panels, strict=False):
        ax.set_facecolor(STYLE["bg"])
        ax.imshow(
...
def diagram_power_spectrum():
    ...
    for px, py in zip(bnx, bny, strict=False):
        ix = int(px * (size - 1))
        iy = int(py * (size - 1))
...
def diagram_radical_inverse():
    ...
    counts = [4, 8, 16]
    for ax, n in zip(axes, counts, strict=False):
        ax.set_facecolor(STYLE["bg"])
...
def diagram_de_casteljau_quadratic():
    ...
    for ax, t_val in zip(axes, t_vals, strict=False):
        ...
        for p, lbl, off in zip(ctrl, labels, offsets, strict=False):
...
def diagram_cubic_tangent_vectors():
    ...
    for p, lbl, off in zip([p0, p1, p2, p3], labels, offsets, strict=False):
        ax.plot(*p, "o", color=STYLE["accent2"], markersize=9, zorder=5)
        ax.text(
            p[0] + off[0],
            p[1] + off[1],
    ...
    for t_val, col in zip(t_samples, colors, strict=False):
...
def diagram_convex_hull():
    ...
    for p, lbl, off in zip([cp0, cp1, cp2, cp3], clabels, coffsets, strict=False):
        ax.plot(*p, "o", color=STYLE["accent2"], markersize=9, zorder=5)
        ax.text(
            p[0] + off[0],
            p[1] + off[1],
...
def diagram_arc_length():
    ...
    for ax, n_seg in zip(axes, segment_counts, strict=False):
