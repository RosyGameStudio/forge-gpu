...
- The lesson was scaffolded with `/dev-start-lesson` and built iteratively
...
1. `/dev-start-lesson` — Scaffold directory + minimal main.c + PLAN.md
2. Build the scene iteratively (already done before this skill)
3. `/dev-create-lesson` — Add README, skill file, screenshot **(this skill)**
...
Follow the same structure as `/dev-new-lesson` creates, but written around the
...
Use the `/dev-add-screenshot` skill to capture and embed the screenshot:
...
Use the `/dev-markdown-lint` skill to verify all markdown passes:
...
- Suggest next steps: `/dev-final-pass` for quality review, then `/dev-publish-lesson`

## What this skill does NOT do

- Does **not** create the lesson directory or main.c (that's `/dev-start-lesson`)
- Does **not** build the scene (that's manual iterative work)
- Does **not** create a branch or PR (that's `/dev-publish-lesson`)
- Does **not** do a full quality review (that's `/dev-final-pass`)
