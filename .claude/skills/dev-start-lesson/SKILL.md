...
Scaffold a new advanced GPU lesson. Unlike `/dev-new-lesson` (which creates a
complete lesson in one pass), `/dev-start-lesson` creates a minimal buildable
...
**When to use `/dev-new-lesson` instead:**
...
1. `/dev-start-lesson` — Scaffold directory + minimal main.c + PLAN.md (this skill)
2. Build the scene iteratively (manual work with agent assistance)
3. `/dev-create-lesson` — Add README, skill file, screenshot to the working scene
4. `/dev-final-pass` — Quality review
5. `/dev-publish-lesson` — Commit and PR
...
- If new math is needed, note it in the PLAN.md (use `/dev-math-lesson` later)
...
- [ ] Directory scaffolded (`/dev-start-lesson`)
- [ ] Scene building (main.c, shaders)
- [ ] New concept implemented
- [ ] README written (`/dev-create-lesson`)
- [ ] Skill file created (`/dev-create-lesson`)
- [ ] Screenshot captured
...
- Does **not** create README.md (that's `/dev-create-lesson`)
- Does **not** create a skill file (that's `/dev-create-lesson`)
- Does **not** capture screenshots (that's `/dev-create-lesson` or `/dev-add-screenshot`)
