...
- Show visual results through diagram assets (matplotlib), not GPU rendering
...
verifying it. UI lesson programs typically:

- Parse or generate font/UI data structures
- Write output images (PNG/BMP) showing glyphs, atlases, or layouts
- Print metrics, bounding boxes, or layout results to the console
...
[Brief description of what the example program produces — output images,
console metrics, or data files]
...
Verify the demo runs and produces expected output (images, console text, or
data files).
...
- **Example output accuracy** — If including example output in README, copy it
  directly from the actual program — don't manually type it.
